...
# to note I tried a number of alternatives eg I figured that that column name with the most accurate classification of the birds would be the Species Abbreviation however despite repeated attempts I could get it down to one count per entry:

data_new2 <- as.data.frame(apply(test, 2, function(x) gsub("\\s+", "", x))) # Removes blanks, however changes count column to character; I address this in the next step.
glimpse(data_new2)
...
```{r}
pattern <-  "[Pp]enguin]"  #"(?i)pengiun"

pattern_id_pengiun <- cleaned_names_cleaned_seabird_data %>% 
  str_detect(pattern) %>% 
  flatten_chr() %>%
  length()

view(cleaned_names_cleaned_seabird_data)
