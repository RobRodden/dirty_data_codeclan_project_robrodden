...
cleaned_halloween_candy_data <- read_csv(here("clean_data/cleaned_halloween_candy_data.csv"))
...
test_HC_country_overview <- test %>% 
  group_by(country) %>% 
  summarise(character_count = n()) %>% 
  arrange(desc(character_count))
head(test_HC_country_overview, 10)
...
view(test)
  ...
  mutate(str_detect(country, pattern = "Can") %>%
  group_by(country) %>% 
  summarise(character_count = n()) %>% 
  arrange(desc(character_count))
